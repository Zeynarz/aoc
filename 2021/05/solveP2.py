...
for line in lineOfVents:
    ...
    if x1 == x2:
        ...
        # Diagonal lines, gradient of all diagonal lines is 1 or -1
        # To find all points that the line segment covers, just start from the lower point,
        # then work your way up. So y will always plus 1, but if the gradient is negative,
        # x will minus 1, and if the gradient is positive, x will plus 1

        m = 0 # Gradient

        m = (y2 - y1) / (x2 - x1)
        if y1 > y2:
            # y2 is lower point, so start from there
            while y1 >= y2:
                allPoints.append((x2,y2))
                x2 += m
                y2 += 1
        
        elif y1 < y2:
            # y1 is lower point , so start from there
            while y1 <= y2:
